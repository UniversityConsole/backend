...
fn can_serialize_struct() {
    ...
    assert!(serde_ddb::to_hashmap(&value).is_ok())
}
...
fn can_deserialize_struct() {
    ...
    value.insert("i".to_string(), AttributeValue::N("5".to_string()));
    value.insert("j".to_string(), AttributeValue::N("12".to_string()));
    let mut intern = HashMap::new();
    intern.insert("k".to_string(), AttributeValue::N("27".to_string()));
    value.insert("intern".to_string(), AttributeValue::M(intern));
    let res: serde_ddb::error::Result<Basic> = serde_ddb::from_hashmap(value);
    assert!(res.is_ok());
}
...
fn can_go_back_and_forth() {
    ...
    let hm = serde_ddb::to_hashmap(&value).unwrap();
    let out: Basic = serde_ddb::from_hashmap(hm).unwrap();
    assert_eq!(value, out);
}
...
fn can_create_struct_custom_serialization() {
    ...
    let hm = serde_ddb::to_hashmap(&value).unwrap();
    let point_result: std::result::Result<Point, serde_ddb::Error> = serde_ddb::from_hashmap(hm);
...
fn can_deserialize_hashset() {
    ...
    value.insert(
        "bar".to_string(),
        AttributeValue::Ss(vec!["foo".to_owned(), "bar".to_owned()]),
    );
    value.insert(
        "baz".to_string(),
        AttributeValue::Ns(vec!["3".to_owned(), "4".to_owned(), "5".to_owned()]),
    );

    let test_hashes: Foo = serde_ddb::from_hashmap(value).unwrap();
...
fn can_be_missing_with_default() {
    ...
    let request: Request = serde_ddb::from_hashmap(value).unwrap();
